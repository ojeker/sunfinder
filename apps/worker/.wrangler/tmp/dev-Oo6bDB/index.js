...
// .wrangler/tmp/bundle-95uKxy/strip-cf-connecting-ip-header.js
...
var AppError = class extends Error {
  ...
  status;
  hint;
  constructor(code, status, message, hint) {
    super(message);
    this.code = code;
    this.status = status;
    this.hint = hint;
...
__name(toJsonError, "toJsonError");
...
var allowlist_default = [
  ...
  "seilpark-balmberg.multipixx.net",
  "inubis.multipixx.net",
  "webcam.wasserflue-aarau.ch",
  "imgproxy.windy.com",
  "images.unsplash.com"
];

// src/index.ts
var BASE_ALLOWLIST = new Set(allowlist_default);
var app = new Hono2();
...
__name(requireParam, "requireParam");
...
function parseAllowlistExtra(input) {
  if (!input)
    return [];
  return input.split(",").map((value) => value.trim()).filter(Boolean);
}
__name(parseAllowlistExtra, "parseAllowlistExtra");
function ensureAllowedHost(url, extra) {
...
__name(ensureAllowedHost, "ensureAllowedHost");
...
__name(createHtmlRewriter, "createHtmlRewriter");
...
// .wrangler/tmp/bundle-95uKxy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-95uKxy/middleware-loader.entry.ts
